...
const AutomatedTrading = () => {
  return (
    ...
    >
      ...
      <div
        id="second_main"
        className="flex flex-col justify-center items-center space-y-8  mx-96"
      >
        ...
        <div id="grid" className="grid grid-cols-2 mx-10 ">
          {list.map((value, iteration) => {
            return (
              <div key={iteration}>
                <div className="flex flex-col justify-start space-y-4 py-4 px-4  h-full w-full">
                  <div>
                    <Image src={value.icon} alt="icon" width={50} height={50} />
                  </div>
                  <div> {value.title}</div>
                  <div className="text-start tracking-wide leading-6 font-light text-sm">
                    {value.text}
                  </div>
                </div>
              </div>
